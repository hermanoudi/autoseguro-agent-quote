...
Prover acesso resiliente à API legada e instável `/quote` (20% de falha, 10% de
lentidão de 8s por chamada, sorteados por chamada) através de um componente único,
`QuoteClient`, responsável por timeout curto, retry com backoff e jitter, circuit
breaker e classificação correta de cada tipo de erro — distinguindo falha transitória,
recusa por regra de negócio e erro de payload/validação — de forma que nenhum preço seja
apresentado ao lead a não ser que tenha vindo de uma resposta 200 real da API.
